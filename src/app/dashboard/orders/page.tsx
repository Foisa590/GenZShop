  ...
  if (orders.length === 0) {
    return (
      <EmptyState
        ...
        title="No orders yet"
        description="অর্ডার নেই • Start shopping to see orders here!"
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-sm shadow-sm p-4">
        ...
        <p className="text-xs text-gray-500 mt-0.5">আমার অর্ডার</p>
      </div>
      {orders.map((order) => {
        const status = ORDER_STATUSES[order.status as keyof typeof ORDER_STATUSES] || ORDER_STATUSES.pending;
        return (
          <div key={order.id} className="bg-white rounded-sm shadow-sm p-4">
            <div className="flex items-center justify-between mb-3 pb-3 border-b">
              ...
              <div className="text-right">
                ...
                <p className="text-sm font-bold mt-1">{formatPrice(order.total_amount)}</p>
              </div>
            </div>
